...
gene_candid = msig %>% 
  filter(gs_name == hallmark_list[1]) %>% 
  select(gene_symbol) %>% 
  unlist() 
```
...
```{r inpudData, eval=FALSE}
# Filter data to hallmark_list[k]
.data  = RNASeq %>% 
...
p = ncol(inpudData)
estimateGraph  = ne.PEN(dat = inpudData, nlambda = 100, ntau = 10, 
                        V = 1:p, order = TRUE, verbose = FALSE)
modelPerformance(estimateGraph,trueGraph)
```


```{r,eval=FALSE}
vGraphM <- matrix(0, nrow = p, ncol = p) 
...
estimateGraph <- mofredsPC(mirror(vGraphM),inpudData,dimRed = TRUE)
```
